...
export default function Attendance() {
  ...
  const stats = getTodayAttendanceStats(attendance, activeStudents.length);
              ...
              >
                ...
                <div className="mt-16">
                  <h4 style={{ fontSize: '0.88rem', marginBottom: 12, color: 'var(--text-secondary)' }}>
                    Students Checked In ({todayPresentStudents.length})
                  </h4>
                  <div style={{ maxHeight: 280, overflowY: 'auto' }}>
                    {todayPresentStudents.length === 0 ? (
                      <div className="empty-state" style={{ padding: '24px 12px' }}>
                        <p>No students checked in yet today</p>
                      </div>
                    ) : (
                      <div className="activity-feed">
                        {todayPresentStudents.map((rec, idx) => (
                          <motion.div
                            key={rec.id}
                          ...
                          >
                            ...
                            <span className={`badge ${rec.status === 'late' ? 'badge-late' : 'badge-present'}`}>
                              {rec.status}
                            </span>
                          </motion.div>
                        ))}
