...
from omr_scanner.config.omr_template import T3_TEMPLATE

def test_t3_synthetic_pipeline(tmp_path):
    blank_path = "OMR_PROJECT/templates/T3_NEET_180/neet_180_template.png"
    if not os.path.exists(blank_path):
        pytest.skip(f"Blank template not found: {blank_path}")
        
    img = cv2.imread(blank_path)
    
    for sec_idx, section in enumerate(T3_TEMPLATE.sections):
    ...
    synthetic_path = str(tmp_path / "synthetic_t3.png")
    cv2.imwrite(synthetic_path, img)
    
    output_dir = str(tmp_path / "output")
    os.makedirs(output_dir, exist_ok=True)
    
    result = run_scanner(synthetic_path, output_dir, template_name="T3", debug=False)
