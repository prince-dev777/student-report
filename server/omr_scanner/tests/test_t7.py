...
from omr_scanner.config.omr_template import T7_TEMPLATE

def test_t7_synthetic_pipeline(tmp_path):
    blank_path = "OMR_PROJECT/templates/T7_OMR_50/omr_50_template.png"
    if not os.path.exists(blank_path):
        pytest.skip(f"Blank template not found: {blank_path}")
        
    img = cv2.imread(blank_path)
    
    for sec_idx, section in enumerate(T7_TEMPLATE.sections):
    ...
    synthetic_path = str(tmp_path / "synthetic_t7.png")
    cv2.imwrite(synthetic_path, img)
    
    output_dir = str(tmp_path / "output")
    os.makedirs(output_dir, exist_ok=True)
    
    result = run_scanner(synthetic_path, output_dir, template_name="T7", debug=False)
