...
from omr_scanner.config.omr_template import T4_TEMPLATE

def test_t4_synthetic_pipeline(tmp_path):
    blank_path = "OMR_PROJECT/templates/T4_NEET_90/neet_90_template.png"
    if not os.path.exists(blank_path):
        pytest.skip(f"Blank template not found: {blank_path}")
        
    img = cv2.imread(blank_path)
    
    for sec_idx, section in enumerate(T4_TEMPLATE.sections):
        ...
        for q_idx in range(num_q):
            ...
            cv2.circle(img, (real_x, real_y), 15, (0, 0, 0), -1)
            
    synthetic_path = str(tmp_path / "synthetic_t4.png")
    cv2.imwrite(synthetic_path, img)
    
    output_dir = str(tmp_path / "output")
    os.makedirs(output_dir, exist_ok=True)
    
    result = run_scanner(synthetic_path, output_dir, template_name="T4", debug=False)
