"""
JEE 75 MCQ Only — OMR Template Generator
Generates: jee_75_mcq_template.png
...
Layout (top to bottom):
  ...
  5. Questions: 3 columns × 25 rows (PHYSICS 1-25, CHEMISTRY 26-50, MATHEMATICS 51-75)
     - 5 blocks of 5 questions each per column
"""
...
# ─────────────────────────────────────────
# 3. Roll No (top-left) + Student Details (right) — side by side
# ─────────────────────────────────────────
def draw_roll_no(img):
    """Roll No grid: 5 cols × 10 rows — upper left corner below title"""
    col_gap = 26.0
    cols_x = [90.0 + c * col_gap for c in range(5)]
    
    first_row_y = 175.0   # Starts right below the title
    row_gap = 20.0
...
def draw_student_details(img):
    """Student details box: right side, next to Roll No"""
    # Right side box — starts after Roll No area
    x1, y1 = 230, 130
    ...
    for i in range(4):
        ...
        if i < 3:
            ...
            # TEST DATE left, INVIGILATOR SIGN slightly towards center-right
            draw_text_left(img, "TEST DATE:", s(x1 + 10), s(label_y), scale=0.33, thickness=1)
            # Invigilator sign — not at extreme right, a bit to the left
            inv_text = "INVIGILATOR SIGN:"
            fs = 0.33 * S
            th = max(1, int(1 * S))
            (tw, _), _ = cv2.getTextSize(inv_text, cv2.FONT_HERSHEY_SIMPLEX, fs, th)
            inv_x = int(s(x2) - tw - s(250))  # 250px padding from right edge — further to the left
            cv2.putText(img, inv_text, (inv_x, int(s(label_y))),
                        cv2.FONT_HERSHEY_SIMPLEX, fs, (0, 0, 0), th, cv2.LINE_AA)

# ─────────────────────────────────────────
# 4. Question Columns (3 × 25)
# ─────────────────────────────────────────
def draw_questions(img):
    """
    3 columns: PHYSICS (1-25), CHEMISTRY (26-50), MATHEMATICS (51-75)
    5 blocks of 5 questions, starting right after Roll No / Details area
    """
    options = ['A', 'B', 'C', 'D']
    bubble_r = 8
    opt_gap = 26.0       # horizontal gap between A B C D
    q_gap = 28.0         # vertical between questions in same block
    block_gap = 46.0     # total gap between blocks
    
    # 3 columns evenly across the page
    page_w = PAGE_RIGHT - PAGE_LEFT
    col_spacing = page_w / 3.0
    columns = [
        {"label": "PHYSICS",     "start": 1,  "cx": PAGE_LEFT + col_spacing * 0.5},
        {"label": "CHEMISTRY",   "start": 26, "cx": PAGE_LEFT + col_spacing * 1.5},
        {"label": "MATHEMATICS", "start": 51, "cx": PAGE_LEFT + col_spacing * 2.5},
    ]
    
    # Start questions below the header area — extra gap so it doesn't look chipka
    y_start = 435.0
    
    # 25 y-positions (5 blocks × 5 rows)
    rows_y = []
    y = y_start
    ...
    for col in columns:
        ...
        # Column header
        draw_text_centered(img, col["label"], s(cx_center), s(y_start - 22), scale=0.42, thickness=2)
...
def generate():
    ...
    out_path = 'jee_75_mcq_template.png'
